...
class BaseModel(object):
    ...
    def __init__(self,
        ...
        shutil.make_archive(self.output_path + '/src', 'zip',
    ...
    def identifier(self):
        ...
        identifier = self.__identifier_stem + self._identifier_suffix
        identifier += (
            '_bs%d_lr%g' %
            (self._batch_size,
             self._learning_schedule[0]['learning_rate'])
        )
        if next(iter(self._train_data.values()))._use_colour:
            identifier += '_colour'
        return identifier
    ...
    def _build_optimizers(self):
        ...
        for spec in self._learning_schedule:
            ...
            assert isinstance(loss_terms, list)
            for loss_term_key in loss_terms:
            ...
            logger.info('Built optimizer for: %s' % ', '.join(loss_terms))
    ...
    def train(self, num_epochs=None, num_steps=None):
        ...
        try:
            ...
            for current_step in range(initial_step, num_steps):
                ...
                loss_term_keys = schedule['loss_terms_to_optimize']
    ...
    def evaluate_for_submission(self, data_source):
        """Evaluate on given test data for online submission."""
        self.initialize_if_not(training=False)
        self.checkpoint.load_all()  # Load available weights

        first_training_data_source = next(iter(self._test_data.values()))
        input_tensors = first_training_data_source.output_tensors

        # Shut down data sources used for training and live-validation
        for ds in (list(self._train_data.values()) + list(self._test_data.values())):
            ds.cleanup()
        ...
        logger.info('Evaluating %d batches for online submission.' % num_batches)
        for i in range(num_batches):
            ...
            for _ in range(current_batch_size):
                ...
                if len(batch_data) == 0:  # initialize lists
                    batch_data = dict([(k, []) for k in preprocess_out.keys()])
                for k, v in preprocess_out.items():
                    batch_data[k].append(v)

            # Pad the last batch as necessary
            if current_batch_size < batch_size:
                difference = batch_size - current_batch_size
                for k, v in batch_data.items():
                    v = np.array(v)
                    pad_width = [(0, difference)] + [(0, 0)] * (v.ndim - 1)
                    batch_data[k] = np.pad(v, pad_width=pad_width, mode='constant',
                                           constant_values=0.0)

            # Finalize dict to feed to NN
            feed_dict = dict([(input_tensors[k], np.asarray(v)) for k, v in batch_data.items()])
            feed_dict[self.is_training] = False
            feed_dict[self.use_batch_statistics] = True

            gazes_batch = self._tensorflow_session.run(
                self.output_tensors['test']['gaze'],
                feed_dict=feed_dict,
            )[:current_batch_size, :]
